...
BEGIN TRY
	DECLARE @PROCESS_ID BIGINT,
					@LOCATION VARCHAR(255) = 'sp_User_UpdatePassword';
	...
	SELECT @CURRENT_PASSWORD = [PASSWORD] FROM [dbo].[TB_M_USER] WHERE [USERNAME] = @USERNAME

	-- Kalau user ini belum pernah punya password lokal (misalnya baru dipindah dari AD_USER),
	-- tidak ada "password lama" untuk dibuktikan -> perlakukan sebagai set password baru,
	-- bukan ganti password, jadi Old Password tidak wajib/divalidasi.
	IF @CURRENT_PASSWORD IS NOT NULL
	BEGIN
		IF @OLD_PASSWORD IS NULL OR LEN(@OLD_PASSWORD) < 1
		BEGIN
			SET @RETURN_MSG = 'ERROR: Old Password not be null';
			EXEC sp_WriteLog @PROCESS_ID, '3', 'ERR', @RETURN_MSG, @LOCATION, @LOGIN_USER
			RETURN 0;
		END
		...
		BEGIN
			SET @RETURN_MSG = 'ERROR: Invalid Old Password';
			EXEC sp_WriteLog @PROCESS_ID, '3', 'ERR', @RETURN_MSG, @LOCATION, @LOGIN_USER
			RETURN 0;
		END
	END
	...
	BEGIN
		SET @RETURN_MSG = 'ERROR: New Password not be null';
		EXEC sp_WriteLog @PROCESS_ID, '3', 'ERR', @RETURN_MSG, @LOCATION, @LOGIN_USER
		RETURN 0;
	END
	...
	BEGIN
		SET @RETURN_MSG = 'ERROR: Confirm Password should be same with New Password';
		EXEC sp_WriteLog @PROCESS_ID, '3', 'ERR', @RETURN_MSG, @LOCATION, @LOGIN_USER
		RETURN 0;
	END
