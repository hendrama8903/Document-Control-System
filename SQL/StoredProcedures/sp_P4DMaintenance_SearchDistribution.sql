	...
	SET @QUERY = 'WITH data AS 
								(
									SELECT ROW_NUMBER() OVER (ORDER BY DISTRIBUTION_ID ASC)  RowNumber
										...
										, ISNULL(A.DEPARTMENT_CODE, C.DEPARTMENT_CODE) DEPARTMENT_CODE
										, ISNULL(A.DEPARTMENT_NAME, C.DEPARTMENT_NAME) DEPARTMENT_NAME
										, ''0'' DIVISION_ID
										, ISNULL(A.DIVISION, C.DIVISION) DIVISION
										, ISNULL(A.DIVISION_NAME, DSYS.DIVISION_NAME) DIVISION_NAME
										, CAST(DISTRIBUTION_DATE as varchar) DATE
										, A.STATUS STATUS
										, B.SYSTEM_VALUE STATUS_DISPLAY
										, CASE WHEN A.STATUS = 1 AND EXISTS (
												SELECT 1 FROM TB_R_PUBLISH_HISTORY PH
												JOIN TB_R_CTRL_DOCUMENT PCT ON PCT.DOCUMENT_CTRL_ID = PH.DOCUMENT_CTRL_ID
												WHERE PCT.DOCUMENT_TRANSACTION_ID = A.DOCUMENT_TRANSACTION_ID
												AND PH.DEPARTMENT_ID = A.DEPARTMENT_ID
