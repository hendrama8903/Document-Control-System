...
BEGIN
	...
	SET @QUERY = 'WITH data AS
						...
						S.NEXT_REVIEW_DATE,
										...
										S.DOCUMENT_CREATOR,
										DCAT.DOCUMENT_NAME,
										DCAT.DOCUMENT_CODE AS CATEGORY_CODE,
										ISNULL((
										...
										AH.CURRENT_SEQ AS CURRENT_APPROVAL_SEQ,
										(SELECT COUNT(*) FROM TB_R_APPROVAL_D ADT WHERE ADT.APPROVAL_ID = S.APPROVAL_ID) AS TOTAL_APPROVAL_SEQ,
										ISNULL(ADCUR.LABEL, '''') AS CURRENT_APPROVAL_LABEL,
										ISNULL(ADCUR.APPROVER, '''') AS CURRENT_APPROVER,
										ISNULL(APPU.FULL_NAME, ISNULL(ADCUR.APPROVER, '''')) AS CURRENT_APPROVER_NAME
									...
									LEFT JOIN TB_R_APPROVAL_H AH
										ON AH.APPROVAL_ID = S.APPROVAL_ID
									LEFT JOIN TB_R_APPROVAL_D ADCUR
										ON ADCUR.APPROVAL_ID = AH.APPROVAL_ID AND ADCUR.WORKFLOW_SEQ = AH.CURRENT_SEQ
									LEFT JOIN TB_M_USER APPU
										ON APPU.USERNAME = ADCUR.APPROVER
									...
									IF ISNULL(@USERNAME, '') <> ''  --@YEAR IS NOT NULL
										...
										IF @DIVISION_COUNT > 0
										BEGIN
											-- Div Head Related Division (SPR/SIPOCOR Level 2) juga harus bisa
											-- menemukan dokumen ini di grid-nya sendiri supaya bisa Mengetahui -
											-- bukan cuma dokumen milik divisinya sendiri (request Hendra
											-- 2026-08-20, ditemukan saat testing: tanpa ini dokumen tidak
											-- pernah muncul untuk Div Head divisi terkait sama sekali).
											SET @QUERY += ' AND (
												S.DIVISION IN (SELECT DIVISION FROM TB_M_USER_POS WHERE USERNAME = '''+ @USERNAME+''')
												OR EXISTS (
													SELECT 1 FROM TB_R_DOCUMENT_RELATED_DIVISION RD
													WHERE RD.DOCUMENT_TRANSACTION_ID = S.DOCUMENT_TRANSACTION_ID
													AND RD.DIVISION_CODE IN (SELECT DIVISION FROM TB_M_USER_POS WHERE USERNAME = '''+ @USERNAME+''' AND POSITION_ID = 4)
												)
											) '
										END
									END
